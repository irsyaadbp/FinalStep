  ...
  return (
    <div className="space-y-3">
      {fields.map((f, i) => {
        const Icon = f.icon;
        const Component =
          f.id.includes("password") || f.type === "password"
            ? PasswordInput
            : Input;
        ...
        return (
          ...
          >
            ...
            {f.render ? (
              f.render({ field: f, register, errors })
            ) : (
              <div className="relative mt-1">
                ...
                <Component
                  ...
                  {...register(f.id)}
                  className={cn(
                    `${Icon ? "pl-10" : ""} h-11 rounded-xl border-2 bg-background shadow-[inset_0_2px_4px_0_rgba(0,0,0,0.04)] focus:shadow-[inset_0_2px_4px_0_rgba(0,0,0,0.04),0_0_0_3px_hsl(var(--primary)/0.12)] transition-shadow text-sm`,
                    errors[f.id] && "border-destructive",
                  )}
                />
              </div>
            )}
